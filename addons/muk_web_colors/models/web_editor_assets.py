...
class ScssEditor(models.AbstractModel):
    ...
    def _get_color_variable(self, content, variable):
        """Extract a color value from SCSS content.

        Looks for patterns like:  $mk_color_brand: #243742;
        The ``variable`` argument is the short name (e.g. ``color_brand``);
        the ``$mk_`` prefix is added here.
        """
    ...
    def _replace_color_variables(self, content, variables):
        """Replace color variable values in SCSS content.

        Each entry in ``variables`` is a dict with keys ``name`` and ``value``.
        ``name`` is the short variable name (e.g. ``color_brand``); the method
        builds the full SCSS variable name ``$mk_<name>`` before substituting.
        """
        for variable in variables:
            scss_var = f'$mk_{variable["name"]}'
            content = re.sub(
                fr'\{scss_var}\:?\s(.*?);',
                f'{scss_var}: {variable["value"]};',
                content,
            )
        return content
